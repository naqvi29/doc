...
def doctorlist():
    try:
        ...
        return render_template("doctorlist.html", status=status, data=users, headers=headers, error=error,
                               role=users[0][6], username=username,address=address)
...
def caseview():
    ...
    if session.get("status") and session.get("userid") and session.get("username") and session.get("role"):
        ...
        print("loginuserid is :"+str(loginuserid))
        # ...
        role2 = session.get("role2")
        patient_id = request.args.get("patientid")
        print("patient id is",patient_id)
        print('role2 is,',role2)
        message = ""
        ...
        # user_rights = cur.fetchone()[0]        
        # return str(user_rights)
        print('user_rights is: ',user_rights)
        cur.close()
        connection.close()
        patientid = "NULL"
        print('role2 is: ',role2)
...
def addpatient2():
    ...
    if session.get("status") and session.get("userid") and session.get("username") and session.get("role"):
        if request.method == "POST":
            ...
            # ali addition 
            # fetch projectid
            cur.execute('''select form_id from form where project_id=%s;''',[p_id])
            form_id = cur.fetchone()
            cur.execute('''Insert into form_entry (form_id, user_id, patient_id) values (%s, %s, %s);''',[form_id, createdby, id])
            connection.commit()
            # end ali addition 
        ...
        if session.get("status") and session.get("userid") and session.get("username") and session.get("role"):
            ...
            if request.method == "POST":
                ...
                if record is None:
                    ...
                    print('formentryid is :', formentryid)
                    for eachrecord in data:
                        fieldid = eachrecord.split('_&&_')[0]
                        value = eachrecord.split('_&&_')[1]
